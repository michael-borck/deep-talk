...
- **Next:**
  - **4.1 — project analysis decision.** Investigation found that the dead code is much smaller than originally thought: ALL 6 main `projectAnalysisService` methods are LIVE and rendered. The actual gap is 5-7 leaf-level fields (`mergedTopics`, `mergedActionItems`, `keyQuotes`, full `evidence[]` arrays) computed but never displayed. Plan: build the small UI to surface the highest-value ones rather than delete computation that's working.
  - Backlog: upload-then-assign-project flow, bulk operations, "analysis done" toast, STT confidence display, original/corrected/speaker-tagged diff view.
