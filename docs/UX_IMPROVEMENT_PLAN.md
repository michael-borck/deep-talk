...
- **Next:**
  - **(a) AI speaker correction** — add a new "AI Correction" button inside the existing SpeakerTaggingModal. Does NOT duplicate the existing CorrectionTrigger (that one handles text spelling/grammar via `validateTranscript`). The new button sends speaker samples to Ollama and asks it to: (1) suggest meaningful labels like "Interviewer"/"Participant"/"Researcher" based on context, (2) identify likely merge candidates when clusters look over-split. User sees the suggestions and can accept/edit/reject. Manual correction still works as before — AI just gives a head start.
  - **(b) Cleanup pass** — delete dead LLM speaker tagging methods, remove @xenova/transformers, drop orphaned settings rows.
  - Then 4.1/4.2 from the original plan: project-level analysis dead code, dual analysis path consolidation.
