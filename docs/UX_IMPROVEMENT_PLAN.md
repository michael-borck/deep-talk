...
- **Next:**
  - **4.1 / 4.2** from the original plan: project-level analysis dead code (decide whether to build UI for theme evolution / cross-transcript patterns or delete the unrendered computation), dual analysis path consolidation (collapse `performAdvancedAnalysis` → `performLegacyAdvancedAnalysis` into a single path now that the `oneTaskAtATime` toggle has no good default).
  - Backlog from earlier: upload-then-assign-project flow, bulk operations, "analysis done" toast, STT confidence display, original/corrected/speaker-tagged diff view.
